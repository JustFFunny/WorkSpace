...
{
    QPointF startPot(LEFT_SPACE,m_longHand/2);
...
void myGauge3::drawHorizontalLine(QPainter *painter)
{
    ...
    QPointF rightPot(width()-LEFT_SPACE,TOP_SPACE+m_longHand);
...
void myGauge3::setValue(qreal value)
{
    ...
    if(m_nMin<=value&&value<=m_nMax)
    {
        ...
        qreal lineWidth=width()-2*LEFT_SPACE;
        qreal ratio=(qreal)m_value/m_length;
        qreal x=lineWidth*ratio;
        qreal newX=x+LEFT_SPACE;
        qreal y=TOP_SPACE+m_longHand-10;
