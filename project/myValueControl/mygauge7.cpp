...
void myGauge7::drawMark(QPainter *painter)
{
    ...
    for(int i=0;i<=100;i++)
    {
        if(i%10==0)
        {
            QPointF leftPot(initX+LONG,initY);
        ...
        {
            QPointF leftPot(initX+OK,initY);
            QPointF rightPot(initX,initY);
            painter->drawLine(leftPot,rightPot);
        }
        else
        {
            QPointF leftPot(initX+SHORT,initY);
            QPointF rightPot(initX,initY);
            painter->drawLine(leftPot,rightPot);
        }
        initY+=increment;
...
void myGauge7::drawBucket(QPainter *painter)
{
    ...
    QPointF bucketTopLeftPot(m_markSpace+MARK_BUCKET_SPACE,m_topSpace);
    QPointF bucketBottomRightPot(m_markSpace+MARK_BUCKET_SPACE+m_bucketWidth,height()-m_topSpace);
