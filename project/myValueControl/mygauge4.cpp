...
void myGauge4::drawIndicator(QPainter *painter)
{
    ...
    QPointF topPot(SPACE+m_currentValue*m_increment,m_topY-TOP_SPACE);
    QPointF bottomPot(SPACE+m_currentValue*m_increment,m_bottomY+TOP_SPACE);
...
{
    painter->save();
    m_markLength=m_middleRect.width()-2*SPACE;
    m_length=m_markLength;
    qreal middleRectY1=m_middleRect.topLeft().y();
    m_topY=middleRectY1+TOP_SPACE;
    qreal middleRectY2=m_middleRect.bottomLeft().y();
    m_bottomY=middleRectY2-TOP_SPACE;

    qreal startX=m_middleRect.x()+SPACE;
    m_startX=startX;
    qreal endX=m_middleRect.bottomRight().x()-SPACE;
    ...
    for(int i=0;i<=100;i++)
    {
        if(i%10==0)
        {
            ...
            QPointF bottomPot(startX,m_topY+LONG);
            painter->drawLine(topPot,bottomPot);

            QPointF topPot1(startX,m_bottomY);
            QPointF bottomPot1(startX,m_bottomY-LONG);
            painter->drawLine(topPot1,bottomPot1);

            QString strValue;
            strValue=tr("%1").arg(value);
            int fontWidth=fontMetrics().width(strValue)/2;
            painter->drawText(QPointF(startX-fontWidth,m_topY+fontHeight+LONG),strValue);
        ...
        {
            QPointF topPot(startX,m_topY);
            QPointF bottomPot(startX,m_topY+OK);
            painter->drawLine(topPot,bottomPot);
            QPointF topPot1(startX,m_bottomY);
            QPointF bottomPot1(startX,m_bottomY-OK);
            painter->drawLine(topPot1,bottomPot1);
        ...
        {
            QPointF topPot(startX,m_topY);
            QPointF bottomPot(startX,m_topY+SHORT);
            painter->drawLine(topPot,bottomPot);
            QPointF topPot1(startX,m_bottomY);
            QPointF bottomPot1(startX,m_bottomY-SHORT);
            painter->drawLine(topPot1,bottomPot1);
        }
        startX+=dx;
    }
