...
QTranslator* SingleApplication::current = 0;
Translators SingleApplication::translatorsRes, SingleApplication::translatorsDir;

SingleApplication::SingleApplication(int &argc, char **argv) : QApplication(argc, argv) , w(NULL) , _isRunning(false)     , _localServer(NULL) {
    // 取应用程序名作为LocalServer的名字
    qDebug()<<"enter the new singalApplication";
...
void SingleApplication::loadTranslations(const QString& dir)
{
...
const QStringList SingleApplication::availableLanguagesRes()
{
        // the content won't get copied thanks to implicit sharing and constness
        return QStringList(translatorsRes.keys());
}

const QStringList SingleApplication::availableLanguagesDir()
{
        // the content won't get copied thanks to implicit sharing and constness
        return QStringList(translatorsDir.keys());
}

void SingleApplication::setLanguage(const QString& locale, QString source)
{
        // remove previous
        if (current)
        {
