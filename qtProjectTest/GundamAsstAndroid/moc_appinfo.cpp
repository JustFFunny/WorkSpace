/****************************************************************************
** Meta object code from reading C++ file 'appinfo.h'
**
** Created: Sat Aug 2 16:57:11 2014
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "dialogs/appinfo.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'appinfo.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_appInfo[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       9,    8,    8,    8, 0x08,
      19,    8,    8,    8, 0x0a,
      42,    8,    8,    8, 0x0a,
      55,    8,    8,    8, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_appInfo[] = {
    "appInfo\0\0install()\0gotWWW(QNetworkReply*)\0"
    "openMarket()\0openMarketPC()\0"
};

void appInfo::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        appInfo *_t = static_cast<appInfo *>(_o);
        switch (_id) {
        case 0: _t->install(); break;
        case 1: _t->gotWWW((*reinterpret_cast< QNetworkReply*(*)>(_a[1]))); break;
        case 2: _t->openMarket(); break;
        case 3: _t->openMarketPC(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData appInfo::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject appInfo::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_appInfo,
      qt_meta_data_appInfo, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &appInfo::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *appInfo::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *appInfo::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_appInfo))
        return static_cast<void*>(const_cast< appInfo*>(this));
    return QDialog::qt_metacast(_clname);
}

int appInfo::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
