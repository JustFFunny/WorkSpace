...
QList<File> *Phone::getFileList()
{
    ...
    if(socket->waitForReadyRead(-1))
    {
        ...
        QStringList tmpinfos = readData.split("\n");
        tmpinfos.removeLast();
        tmpinfos.removeLast();
        int size = tmpinfos.size();
        if(size > 0)
        {
            for(int i = 0; i < tmpinfos.size(); i++)
            {
                QStringList info = tmpinfos[i].split(":");
                tmpFile.fileName = info.at(0);
                tmpFile.fileSize = info.at(1);
                tmpFile.fileDate = info.at(2);
                tmpFile.filePath = info.at(3);
                tmpFile.fileType = info.at(4);
                tmpFile.fileColor = QColor(Qt::black);
                QFile plik;
                if (tmpFile.fileType == "file" || tmpFile.fileType == "device")
                {
                    plik.setFileName(QDir::currentPath()+"/tmp/"+tmpFile.fileName);
                ...
                if (tmpFile.fileName == "." || tmpFile.fileName == "..")
                    continue;
                else if(tmpFile.fileName.at(0) == '.' && !this->hiddenFiles)
                    continue;
                else
...
//    if (this->hiddenFiles)
//        command="\""+this->sdk+"\""+"adb shell \"/data/local/tmp/busybox/ls -l -a \'"+this->codec->toUnicode(this->getPath().toUtf8())+"\'\"";
//    else
//        command="\""+this->sdk+"\""+"adb shell \"/data/local/tmp/busybox/ls -l \'"+this->codec->toUnicode(this->getPath().toUtf8())+"\'\"";
...
QList<File> *Phone::getFileList(QString filter)
{
    ...
    qDebug()<<"Phone::getFileList() - "<<this->getPath();
    if (this->hiddenFiles)
        command="\""+this->sdk+"\""+"adb shell \"/data/local/tmp/busybox/ls -l -a \'"+this->codec->toUnicode(this->getPath().toUtf8())+"\' | grep \'" + filter + "\'\"";
    else
        command="\""+this->sdk+"\""+"adb shell \"/data/local/tmp/busybox/ls -l \'"+this->codec->toUnicode(this->getPath().toUtf8())+"\' | grep \'" + filter + "\'\"";
...
FileList *Phone::getStaticFileList(QString path, QString sdk, bool hiddenFiles)
{
    ...
    if (hiddenFiles)
        command="\""+sdk+"\""+"adb shell \"/data/local/tmp/busybox/ls -l -a \'"+codec->toUnicode(path.toAscii())+"\'\"";
    else
        command="\""+sdk+"\""+"adb shell \"/data/local/tmp/busybox/ls -l \'"+codec->toUnicode(path.toAscii())+"\'\"";
...
bool Phone::makeDir(QString newDir)
{
    bool result = false;
    QTcpSocket *socket = new QTcpSocket;
    QString mesg = "DIRECTORY:CREATE:"+newDir+"\n";
    socket->connectToHost(ip,port,QTcpSocket::ReadWrite);
    if (socket->waitForConnected(2000))
    {
        socket->write(mesg.toStdString().c_str(), strlen(mesg.toStdString().c_str()));
    ...
    {
        QString readData = QString::fromUtf8(socket->readAll());
        QString tmpinfos = readData.split("\n")[0];
        if(tmpinfos.contains("CREATE SUCCESS"))
            result = true;
    }
    socket->disconnectFromHost();
    socket->close();
...
bool Phone::remove(QString path)
{
//    if ((this->getConnectionState() != RECOVERY) && (this->getConnectionState() != DEVICE))
//        return false;
    ...
    if(socket->waitForReadyRead(-1))
    {
        QString readData = QString::fromUtf8(socket->readAll());
        QString tmpinfos = readData.split("\n")[0];
        if(tmpinfos.contains("DELETE SUCCESS"))
            result = true;
    }
    socket->disconnectFromHost();
    socket->close();
...
bool Phone::rename(QString oldName, QString newName)
{
    bool result = false;
    QTcpSocket *socket = new QTcpSocket;
    QString mesg = "FILE:RENAME:"+oldName+":"+newName+"\n";
    socket->connectToHost(ip,port,QTcpSocket::ReadWrite);
    if (socket->waitForConnected(2000))
    {
        socket->write(mesg.toStdString().c_str(), strlen(mesg.toStdString().c_str()));
    ...
    if(socket->waitForReadyRead(-1))
    {
        QString readData = QString::fromUtf8(socket->readAll());
        QString tmpinfos = readData.split("\n")[0];
        if(tmpinfos.contains("RENAME SUCCESS"))
            result = true;
    }
    socket->disconnectFromHost();
    socket->close();
...
//    QString operation = "\""+sdk+"\""+ "adb shell su -c '/data/busybox/busybox grep gmail-ls /data/system/sync/accounts.xml'";
//su -c '/data/busybox/busybox grep gmail-ls /data/system/sync/accounts.xml'
    ...
    {
        operation = "\""+sdk+"\""+ "adb shell su -c '/data/busybox/busybox grep gmail-ls /data/system/sync/accounts.xml'";
        proces = new QProcess;
        proces->start(operation);
        proces->waitForFinished(-1);
        output = proces->readAll();
...
{//? (192.168.2.1) at d8:5d:4c:de:85:94 [ether]  on tiwlan0
    QProcess *proces=new QProcess;
    QSettings settings;
    proces->start("\"" + settings.value("sdkPath").toString() + "\"adb shell /data/local/tmp/busybox/arp");
    QString tmp;
    proces->waitForFinished(-1);
    tmp = proces->readAll();
    ...
    proces->start("\"" + settings.value("sdkPath").toString() + "\"adb shell /data/local/tmp/busybox/ifconfig "+tmp);
    proces->waitForFinished(-1);
    tmp = proces->readAll();
