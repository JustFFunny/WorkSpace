...
void ThreadBackup::run()
{
    ...
    QByteArray ba;
    App app;
    proces->start("\"" + this->sdk + "\"" + "adb shell mkdir /sdcard/Gundam/backup");
...
//        version = this->appList.version.takeFirst();
        ...
        proces->start("\""+this->sdk+"\""+"adb shell echo -e \"app.name="+codec->toUnicode(app.appName.toUtf8())+"\" > /sdcard/Gundam/backup/"+app.packageName+".txt");
        proces->waitForFinished(-1);
        proces->start("\""+this->sdk+"\""+"adb shell echo -e \"app.size="+app.appSize+"\" >> /sdcard/Gundam/backup/"+app.packageName+".txt");
        proces->waitForFinished(-1);
        proces->start("\""+this->sdk+"\""+"adb shell echo -e \"app.version="+app.appVersion+"\" >> /sdcard/Gundam/backup/"+app.packageName+".txt");
        proces->waitForFinished(-1);
        ...
        ikona.close();
        proces->start("\""+this->sdk+"\""+"adb push \""+QDir::currentPath()+"/icons/"+app.packageName+".png\" /sdcard/Gundam/backup/");
        proces->waitForFinished(-1);
        ...
        if (this->withData)
        {
            proces->start("\""+this->sdk+"\""+"adb shell /data/local/tmp/busybox/tar -zcf /sdcard/Gundam/backup/"+app.packageName+".DATA.tar.gz /data/data/"+app.packageName);
            proces->waitForFinished(-1);
            output = proces->readAll();
            qDebug()<<"Backup app - "<<output;
        }
        if (this->withApk)
        {
            proces->start("\""+this->sdk+"\""+"adb shell /data/local/tmp/busybox/cp "+app.appFile+ " /sdcard/Gundam/backup/"+app.packageName+".apk");
            proces->waitForFinished(-1);
            output = proces->readAll();
            qDebug()<<"Backup app - "<<output;
        }
...
//        package = this->appList.package.takeFirst();
        ...
        if (this->withApk)
        {
            proces->start("\""+this->sdk+"\""+"adb shell pm install /sdcard/Gundam/backup/" +app.packageName + ".apk");
            proces->waitForFinished(-1);
            output = proces->readAll();
            qDebug()<<"Restore pm - "<<output;
            if (output.contains("Failure [INSTALL_FAILED_INSUFFICIENT_STORAGE]"))
            {
                proces->start("\""+this->sdk+"\""+"adb shell cp /sdcard/Gundam/backup/" +app.packageName + ".apk /data/local/tmp/");
                proces->waitForFinished(-1);
                output = proces->readAll();
                ...
                output = proces->readAll();
                qDebug()<<"Restore pm - "<<output;
                proces->start("\""+this->sdk+"\""+"adb shell rm -f /data/local/tmp/" +app.packageName + ".apk");
                proces->waitForFinished(-1);
            }
            ...
            else if (output.contains("Success"), "")
            {
                if (this->withData)
                {
                    proces->start("\""+this->sdk+"\""+"adb shell rm -rf /data/data/"+app.packageName);
                    proces->waitForFinished(-1);
                    output = proces->readAll();
                    qDebug()<<"Restore rm - "<<output;
                    proces->start("\""+this->sdk+"\""+"adb shell /data/local/tmp/busybox/tar -xzf /sdcard/Gundam/backup/"+app.packageName+".DATA.tar.gz -C /");
                    proces->waitForFinished(-1);
                    output = proces->readAll();
                    qDebug()<<"Restore tar - "<<output;
                    proces->start("\""+this->sdk+"\""+"adb shell \"cat /data/system/packages.xml|/data/busybox/busybox grep '^<package.*"+app.packageName+"'\"");
                    proces->waitForFinished(-1);
                    output = proces->readAll();
                    ...
                    if (start > 7)
                    {
                        ...
                        proces->start("\""+this->sdk+"\""+"adb shell chown -R "+userId+":"+userId+" /data/data/"+app.packageName);
                        proces->waitForFinished(-1);
                        output = proces->readAll();
                        qDebug()<<"Restore chown - "<<output;
                        proces->start("\""+this->sdk+"\""+"adb shell chmod -R 775 /data/data/"+app.packageName);
                        proces->waitForFinished(-1);
                        output = proces->readAll();
