    ...
    ui(new Ui::RecoveryWidget)
{
    ...
    nandroid->start("\"" + sdk + "\"adb shell /data/local/tmp/busybox/find /sbin -name nandroid-mobile*.sh");
    ...
    process->start("\""+sdk+"\"adb shell /data/local/tmp/busybox/ls /sbin/wipe");
    ...
    process->start("\""+sdk+"\"adb shell /data/local/tmp/busybox/ls /sbin/ums_toggle");
    process->waitForFinished(-1);
    tmp = process->readAll();
    if (tmp.contains("No such file"))
    ...
    process=new QProcess();
    process->start("\""+sdk+"\"adb shell /data/local/tmp/busybox/ls /sbin/fix_permissions");
    process->waitForFinished(-1);
    tmp = process->readAll();
    if (tmp.contains("No such file"))
