...
void MessageWidget::readFinshed()
{
    ts->start();
}
...
void MessageWidget::sendToClient(QString mesg)
{
    if(ts->isRunning())
        ts->exit();
...
void MessageWidget::sendSmsThread(QString message)
{
    ...
    if (number == "")
        return;
    sendToClient("SMS:SEND:"+number.toLatin1()+":"+message.toLatin1()+"\n");
...
void MessageWidget::sendSms(QString number,QString message)
{
    ...
    //addSMS("1", "1",QString::number(QDateTime::currentMSecsSinceEpoch()),number,"1","outbox",message);
}
...
void MessageWidget::sort(QString threadId)
{
    QString ss = threadId;
    int a = 10;
    int b = a+1-0;
