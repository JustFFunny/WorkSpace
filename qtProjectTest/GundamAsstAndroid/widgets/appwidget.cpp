...
//    this->ui->editCyrketVersion->hide();
    ...
    this->ui->buttonOpenMarket->hide();
    this->ui->toolButtonBackup->hide();
    this->ui->buttonAppBrain->hide();
...
//    this->phone->setPath("/");
//    this->phone->makeDir("sdcard/Gundam/backup");
...
//    }
    ...
    this->phone->setPath("/sdcard/Gundam/backup/");
...
void ThreadBackups::run()
{
    ...
    proces->start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls /sdcard/Gundam/backup/*.txt");
    ...
    while (outputLines.size() > 0)
    {
        ...
        proces->start("\"" + this->sdk + "\"adb shell cat /sdcard/Gundam/backup/"+backupFound.packageName+".txt");
        proces->waitForFinished(-1);
        output = proces->readAll();
        if (!settings.contains("apps/"+backupFound.packageName+"/icon"))
        {
            proces->start("\"" + this->sdk + "\"adb pull /sdcard/Gundam/backup/"+backupFound.packageName+".png "+QDir::currentPath()+"/icons/"+backupFound.packageName+".png");
            proces->waitForFinished(-1);
...
//        backupFound.appIcon = QIcon(pix);
        ...
        proces->start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls  /sdcard/Gundam/backup/"+backupFound.packageName+".apk");
        proces->waitForFinished(-1);
        output = proces->readAll();
        output.remove(QString("%1[0m").arg( QChar( 0x1b )));
        output.remove(QChar( 0x1b ), Qt::CaseInsensitive);
        ...
        proces->start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls /sdcard/Gundam/backup/"+backupFound.packageName+".DATA.tar.gz");
        proces->waitForFinished(-1);
        output = proces->readAll();
        output.remove(QString("%1[0m").arg( QChar( 0x1b )));
        output.remove(QChar( 0x1b ), Qt::CaseInsensitive);
...
void ThreadApps::run()
{
    ...
    if (this->systemApps)
    {
        proces.start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls -l /system/app/*.apk");
        proces.waitForFinished(-1);
        output = proces.readAll();
    ...
    {
        proces.start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls -l /data/app/*.apk");
        proces.waitForFinished(-1);
        output = proces.readAll();
        ...
        proces.start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls -l /data/app-private/*.apk");
        proces.waitForFinished(-1);
        output = proces.readAll();
        qDebug()<<"Get apps data - "<<output;
        lines = output.split("\n", QString::SkipEmptyParts);
        ...
        proces.start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls -l /mnt/asec/*/*.apk");
        proces.waitForFinished(-1);
        output = proces.readAll();
...
//                        sdFolder = split.at(2);
        ...
        if (!sdFolder.isEmpty() && !sdFolder.contains(QRegExp("<.*>")))
        {
            if (!sdFolder.endsWith("/",Qt::CaseInsensitive))
                sdFolder.append("/");
            proces.start("\"" + this->sdk + "\"adb shell /data/local/tmp/busybox/ls -l "+ sdFolder + "/*/*.apk");
            proces.waitForFinished(-1);
...
//            }
    ...
    zip.start("\""+sdk+"\""+"adb shell mkdir /sdcard/Gundam");
    zip.waitForFinished(-1);
    zip.start("\""+sdk+"\""+"adb shell mkdir /sdcard/Gundam/tmp");
    zip.waitForFinished(-1);
