...
//        file->remove();
    ...
    {
        firstFileName = file->read(qbytearrayToInt16(fileNameLength));
        do{
            filePos = file->pos();
    ...
    ui(new Ui::FileWidget)
{
    ...
    this->phone=new Phone(this->sdk,false);
    ...
    //ui->pushButtonCopy->hide();
    //ui->pushButton->hide();
    //ui->pushButtonHiddenFiles->hide();
...
void FileWidget::keyPressEvent(QKeyEvent *e)
{
        ...
        {
            case Qt::Key_F7:
                on_pushButtonRefresh_2_pressed();
                break;
            case Qt::Key_F8:
...
void FileWidget::leftDisplay()
{
    if (this->leftMode=="computer")
    {
        ...
        //this->computer->setHiddenFiles(this->settings->computerHiddenFiles);
        this->computer->setHiddenFiles(false);
        this->leftTableView->horizontalHeader()->setVisible(this->settings->showComputerColumnsHeaders);
    ...
    else
    {
       ...
       // this->phoneLeft->setHiddenFiles(this->settings->phoneHiddenFiles);
        this->phoneLeft->setHiddenFiles(false);
...
void FileWidget::on_pushButtonRefresh_2_pressed()
{
    this->leftRefresh();
}
...
void FileWidget::phoneDelete()
{
    ...
    {
        index = sortModel->mapToSource(indexList.takeFirst());
        if(!phoneTmp->remove(fileModel->getFile(index.row()).filePath))
        {
...
void FileWidget::phoneNewDir()
{
    ...
    QString path = phoneTmp->getPath()+"new dir";
    if (!phoneTmp->makeDir(path))
    {
...
void FileWidget::phoneRenameSlot(QModelIndex indexS, QModelIndex indexE)
{
    ...
    if (indexS == indexE)
    {
        ...
        QString newFile = phoneTmp->getPath()+indexS.data(Qt::DisplayRole).toString();
        //phoneTmp->rename(oldFile, newFile);
        if (!phoneTmp->rename(oldFile, newFile))
        {
            QMessageBox::information(this,tr("error"),tr("rename failed.\nMake sure that you are allowed to do this"),QMessageBox::Ok);
        }
        if (this->leftTableView->hasFocus())
        {
            this->leftDisplay();
...
void FileWidget::rightDisplay()
{
    ...
    //this->phone->setHiddenFiles(this->settings->phoneHiddenFiles);
    this->phone->setHiddenFiles(false);
    this->rightTableView->horizontalHeader()->setVisible(this->settings->showPhoneColumnsHeaders);
...
void ThreadFind::run()
{
    ...
    QString path, file;
    QStringList strList;
    proces->start("\""+this->sdk+"\"adb shell /data/local/tmp/busybox/find "+this->path+" -iname \'*"+this->fileName+"*\'");
