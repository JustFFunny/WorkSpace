...
{
    if (QMessageBox::question(this,tr("Associate??"),tr("Are you sure you want to associate *.apk files with Gundam???"),QMessageBox::Ok | QMessageBox::No) == QMessageBox::No)
        return;

    QSettings settings("HKEY_LOCAL_MACHINE\\SOFTWARE\\Classes\\", QSettings::NativeFormat);
    settings.setValue(".apk/Default", "Gundam.File.apk");
    QString tmp;
    tmp = qApp->arguments().at(0);
    tmp.replace("/", "\\");
    settings.setValue("Gundam.File.apk/DefaultIcon/Default", tmp + ",0");
    settings.setValue("Gundam.File.apk/shell/open/command/Default",
                      "\"" + tmp + "\" \"%1\"");
    settings.setValue("Gundam.File.apk/shell/Install/Default",
                      tr("Install apk"));
    settings.setValue("Gundam.File.apk/shell/Install/command/Default",
                      "\"" + tmp + "\" \"-install\" \"%1\"");
}
...
void SettingsWidget::detectSdExtFolder()
{
        ...
        shell->start("\"" + this->sdkPath + "\"adb shell '/data/local/tmp/busybox/stat /data/app |grep \"File\"'");
